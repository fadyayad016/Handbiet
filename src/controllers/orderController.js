...
exports.payment = asyncHandler(async (req, res) => {
  const payment = await orderService.createPaymentIntentForOrders(
    req.user,
    req.body
  );
...
exports.updateNotificationReadStatus = asyncHandler(async (req, res) => {
  const updatedNotification = await orderService.updateNotificationReadStatus(
    req.user,
    req.body
  );
...
exports.getOrderByIdForCustomer = asyncHandler(async (req, res) => {
  ...
  const order = await orderService.getOrderByIdForCustomer(userId, orderId);
  if (!order) {
